...
//! run inside `wrangler dev --local` against a local SQLite-backed Durable
//! Object.
//!
//! Skipped unless `DO_WORKER_URL` names a running test Worker, so plain
//! `cargo test` stays offline. `make test-worker` builds the Worker, starts it
//! on fresh storage and runs this with the URL set.
...
#[test]
fn every_scenario_passes_against_a_local_durable_object() {
    let Ok(base) = std::env::var("DO_WORKER_URL") else {
        eprintln!("DO_WORKER_URL is unset; skipping -- run `make test-worker`");
        return;
    };
