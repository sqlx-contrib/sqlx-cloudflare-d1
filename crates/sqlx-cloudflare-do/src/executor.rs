...
use crate::{js, Do, DoArguments, DoConnection, DoQueryResult, DoRow, DoStatement, DoTypeInfo};
...
// D1's `SendFuture`: they are `Send` because `DoConnection` is `Sync`. They
// still run the query when polled, not when built, as sqlx's futures do.
impl<'c> Executor<'c> for &'c DoConnection {
    type Database = Do;
    ...
    fn execute_many<'e, 'q: 'e, E>(self, query: E) -> BoxStream<'e, Result<DoQueryResult, Error>>
    where
        'c: 'e,
        E: 'q + Execute<'q, Do>,
    {
    ...
    fn fetch_optional<'e, 'q: 'e, E>(self, query: E) -> BoxFuture<'e, Result<Option<DoRow>, Error>>
    where
        'c: 'e,
        E: 'q + Execute<'q, Do>,
    {
...
impl<'c> Executor<'c> for &'c mut DoConnection {
    type Database = Do;
