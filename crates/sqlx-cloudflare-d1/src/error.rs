...
/// An error D1 returned for a statement.
///
/// D1 reports no structured result code across the binding, only SQLite's
/// message as text, which ends with the code by name:
/// `NOT NULL constraint failed: users.email: SQLITE_CONSTRAINT (extended:
/// SQLITE_CONSTRAINT_NOTNULL)`. [`code`](DatabaseError::code) is that name,
/// and [`kind`](DatabaseError::kind) is read from it.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct D1DatabaseError {
    message: String,
}

impl D1DatabaseError {
    pub(crate) fn new(message: String) -> Self {
        Self { message }
    }
}

impl Display for D1DatabaseError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.write_str(&self.message)
    }
}

impl StdError for D1DatabaseError {}

impl DatabaseError for D1DatabaseError {
    fn message(&self) -> &str {
        &self.message
    }

    /// SQLite's result code by name -- the extended one when D1 gives it, as
...
/// The result code at the end of a D1 message: the `extended:` one if there
...
/// is brittle -- which is why both live here and nowhere else: when D1
...
mod tests {
    ...
    use super::D1DatabaseError;

    fn kind(message: &str) -> ErrorKind {
        D1DatabaseError::new(message.to_owned()).kind()
    }
    ...
    fn reads_the_extended_code() {
        let error = D1DatabaseError::new(NOT_NULL.to_owned());
