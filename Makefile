...
#
# `make test` needs no Worker: sqlx-cloudflare-d1's tests/driver.rs skips
# itself unless D1_WORKER_URL is set. `make test-worker` builds and serves the
# test Worker and sets it.
...
# Cloudflare account involved -- see run.sh next to the test Worker.
.PHONY: test-worker
test-worker:
